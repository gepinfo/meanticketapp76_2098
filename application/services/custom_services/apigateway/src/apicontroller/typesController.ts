...
this.router.get('/types/:id', this.GpGetNounById);
this.router.get('/types', this.GpGetAllValues);
this.router.post('/types', this.GpCreate);
this.router.get('/types/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
    }
...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.SERVICETYPESURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.SERVICETYPESURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.SERVICETYPESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpGetNounById(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into typesController.ts: GpGetNounById');
        new ApiAdapter().get(Constant.SERVICETYPESURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from typesController.ts: GpGetNounById');
        }).catch(err => {
            res.send(err);
        });
    }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.SERVICETYPESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
